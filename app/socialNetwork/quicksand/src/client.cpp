...
bool Client::serve_req(nu::PerfThreadState *perf_state,
  ...
  try {
    ...
    if (user_timeline_req) {
      state->service.run(&BackEndService::ReadUserTimeline,
    ...
    if (home_timeline_req) {
      state->service.run(&BackEndService::ReadHomeTimeline,
