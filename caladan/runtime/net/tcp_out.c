			       ...
			       uint16_t len)
{
	...
	return ipv4_phdr_cksum(CALADAN_IPPROTO_TCP, local_ip, remote_ip, len);
}
...
int tcp_tx_raw_rst(struct netaddr laddr, struct netaddr raddr, tcp_seq seq)
{
	...
	/* transmit packet */
	ret = net_tx_ip(m, CALADAN_IPPROTO_TCP, raddr.ip, DEFAULT_DSCP);
	if (unlikely(ret))
		mbuf_free(m);
	return ret;
}
...
int tcp_tx_ack(tcpconn_t *c)
{
	...
	ret = net_tx_ip(m, CALADAN_IPPROTO_TCP, c->e.raddr.ip, c->dscp);
	if (unlikely(ret))
		mbuf_free(m);
	return ret;
}
